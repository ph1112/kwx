...
exports.start = function(config,mgr){
	...
	io.sockets.on('connection',function(socket){
		socket.on('login',function(data){
            ...
            if(!roomInfo){
                socket.emit('login_result',{errcode:4,errmsg:"room is not exists"});
                return;
            }
		...
		socket.on('liangDao',function(data){
			console.log('liangDao');
			if(socket.userId == null){
				return;
			}
			...
			socket.gameMgr.tingpai(socket.userId,data);
		});
		...
		socket.on('ting',function(data){
			...
			var ting = data;
			socket.gameMgr.tingPai(socket.userId,ting);
		});
		...
		socket.on('dissolve_agree',function(data){
			...
			if(ret != null){
				...
				var doAllAgree = true;
				for(var i = 0; i < 3; ++i){//dr.states.length
		...
		socket.on('disconnect',function(data){
			...
			var userInfo = userMgr.get(userId);
			if(userInfo != socket){
				return;
			}
			var data = {
