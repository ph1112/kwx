cc.Class({
    ...
    onLoad: function () {
        ...
        this._flyfenLabel = this.node.getChildByName("flyfenLabel");
        if(this._flyfenLabel){
            this._flyfenLabel.active = false;
        }
    ...
    onIconClicked:function(){
        ...
        if(this._userId != null && this._userId > 0){
           ...
           var array=cc.vv.gameNetMgr.seats;
            ...
            //var seindex=cc.vv.gameNetMgr.getSeatIndexByID(this._userId);
            var seindex = cc.vv.gameNetMgr.getLocalIndex(seat.seatindex);
          if(seindex==0)
          {
            cc.vv.userinfoShow.show(seat.name,seat.userid,iconSprite,sex,seat.ip); 
          }
          else{
              cc.vv.userinfoShow.showTwo(array,seat.userid,3); 
          }           
        }
    },
    
     piaoFeng:function(changeScore){
        var labelshow = cc.sequence(
        ...
        this._flyfenLabel.active = true;
        console.log("----piaoFeng: ",changeScore);
        if (changeScore > 0){
            //this._flyfenLabel.getComponent(cc.Label).font = this.addFont;
            this._flyfenLabel.getComponent(cc.Label).string = "+" + changeScore.toString();
        }else{
           //this._flyfenLabel.getComponent(cc.Label).font = this.mulFort;
            this._flyfenLabel.getComponent(cc.Label).string = changeScore.toString();
        }
        

        this._flyfenLabel.runAction(labelshow);
