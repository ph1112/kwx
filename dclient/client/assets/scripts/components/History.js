...
cc.Class({
    ...
    onLoad: function () {
        ...
        this._cahxun=this._history.getChildByName("chaxun");
        this.addClickEvent(this._cahxun,this.node,"History","onBtnChaxunClicked");

        this._editbox=this._history.getChildByName("editbox");
    ...
    onBtnChaxunClicked:function(){
        cc.vv.audioMgr.playSFX("ui_click.mp3");
        this._history.active = true;
        var string=this._editbox.getComponent(cc.EditBox).string;
        if(!this.isDigit(string,8)){
            this.OpacityShow(this._history,{text:errorTips.chaXunError});
            return;
        }
        var self = this;
        cc.vv.userMgr.getHuiFangRooms(function(data){
            data.sort(function(a,b){
                return a.time < b.time; 
            });
            self._historyData = data;
    ...
    OpacityShow:function(parentNode,o){
        if(!this._lab){
            ...
            this._lab.node.setPosition(20,-270);
    ...
    initRoomHistoryList:function(data){
        for(var i = 0; i < data.length; ++i){
            ...
            node.getChildByName("roomNo").getComponent(cc.Label).string =  data[i].id;//"房间ID:" +
    ...
    initGameHistoryList:function(roomInfo,data){
        ...
        for(var i = 0; i < data.length; ++i){
            ...
            var titleId = "" + (idx + 1);
            node.getChildByName("title").getComponent(cc.Label).string = titleId;
            node.getChildByName("roomNo").getComponent(cc.Label).string =  roomInfo.id;//"房间ID:" +
