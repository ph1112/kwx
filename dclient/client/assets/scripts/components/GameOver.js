cc.Class({
    ...
    onLoad: function () {
        ...
        this._isZimo=false;
        console.log("this._isZimo1"+this._isZimo);
    ...
    playAni:function(array)
    {
		...
		if(array.length==0)
		{
                    console.log("this._isZimo2"+this._isZimo);
    ...
    hutypeOver:function () {
         ...
         if(cc.vv.gameNetMgr.getMaiMa()==0)
        {
            this.scheduleOnce(this.setgameover,3);//没有买马，
                    ...
                    console.log("this._isZimo3"+this._isZimo+"liangdao"+this._isLiangdao);
            if(cc.vv.gameNetMgr.getMaiMa()==1&&this._isZimo==false)//自摸买马是别人放炮
            {
                this.scheduleOnce(this.setgameover,3);
            }else if(cc.vv.gameNetMgr.getMaiMa()==2)//亮倒自摸买吗没有亮倒或者不是自摸
            {
                if(this._isZimo==false||this._isLiangdao==false){
                this.scheduleOnce(this.setgameover,3);
                }
                else
                {
                this.scheduleOnce(this.daipai,5);
                }
            }else if(this.maiMacard<0) //选码没有中码
            {
    ...
    onGameOver_XZDD(data1){
       ...
       // this._gameover.active = true;
        ...
        for(var i = 0; i < 3; ++i){ 
            ...
            for(var j = 0; j < userData.actions.length; ++j){
                ...
                if(ac.type == "zimo" || ac.type == "ganghua" || ac.type == "dianganghua" || ac.type == "hu" || ac.type == "gangpaohu" || ac.type == "qiangganghu" || ac.type == "chadajiao"){
                    ...
                    else if(ac.type == "gangpaohu"||ac.type == "maozhuanyu"){
                        actionArr.push("杠上炮");
                    }
                    ...
                    console.log("this._isZimo4"+this._isZimo);
                    this._isZimo=false;
                    actionArr.push("放炮");
                }
            ...
            actionArr.push("漂分 x"+userData.piao);
             ...
             console.log("  userData.shukan   "+userData.shukan);
            if(userData.shukan>0){
                 console.log("  kanfen   "+userData.shukan);
                 actionArr.push("数坎 x" + userData.shukan);
            }

            seatView.reason.string = actionArr.join("、");
            
            if(this.maiMacard==-1||cc.vv.gameNetMgr.getMaiMa()==0){
              this._over_Ma.active=false;
            }
            else if(this._isZimo==true&&cc.vv.gameNetMgr.getMaiMa()==1){
                        console.log("this._isZimo4"+this._isZimo);
            ...
            }else if(this._isZimo==true&&cc.vv.gameNetMgr.getMaiMa()==2&&this._isLiangdao==true)
            {
                console.log("maimapai"+userData.maimapai);
                this._over_Ma.active=true;
                this._over_Ma.getChildByName("Sprite").getComponent(cc.Sprite).spriteFrame=
              cc.vv.mahjongmgr.getMahjongSpriteByID_ming("0",userData.maimapai);
            }
        ...
        if(this._isHu)
            {
                        console.log("this._isZimo5"+this._isZimo);
                this.playAni(this.numberArr);
            }else
            {
                this.setgameover();
            }
