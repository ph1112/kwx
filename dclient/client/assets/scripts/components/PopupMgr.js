cc.Class({
    ...
    onLoad: function () {
        ...
        console.log("88:"+this._popuproot );
        this._settings = cc.find("Canvas/popups/settings");
        console.log("99:"+this._settings);
        ...
        this.addBGhandler("settings/bg1/Button1");//背景1
        this.addBGhandler("settings/bg2/Button2");//2
    ...
    addBGhandler:function(btnName)
    {
         var btn = cc.find("Canvas/popups/" + btnName);
        this.addClickEvent(btn,this.node,"PopupMgr","onBtnBGClicked");
    },
    ...
    onBtnBGClicked:function(event)
    {    cc.vv.audioMgr.playSFX("ui_click.mp3");
        var btnName = event.target.name;
         if(btnName=="Button1")
        {
            cc.find("Canvas/bg/Z_backgroud1").active=true;
            cc.find("Canvas/bg/Z_backgroud2").active=false;
			cc.sys.localStorage.setItem("bg",0);
        }else if(btnName=="Button2")
        {
            cc.find("Canvas/bg/Z_backgroud1").active=false;
            cc.find("Canvas/bg/Z_backgroud2").active=true;
			cc.sys.localStorage.setItem("bg",1);
        }
    },
    closeAll:function(){
