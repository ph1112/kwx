...
cc.Class({
	...
	showBg:function()
	{
		var userData = JSON.parse(cc.sys.localStorage.getItem("bg"));
		if(userData){
					 cc.find("Canvas/bg/Z_backgroud1").active=0==userData?true:false;
         cc.find("Canvas/bg/Z_backgroud2").active=1==userData?true:false;
		}
	...
	initEventHandlers: function () {
		...
		this.node.on('game_mopai', function (data) {
			...
			//如果亮倒。自己出牌\
			if(cc.vv.gameNetMgr.getSelfData().liangDao){
				setTimeout(function(){
					self.shoot(pai);
				},850);
			}
		...
		if (data && (data.hu || data.gang || data.peng)) {
			...
			if (data.hu) {
				if(cc.vv.gameNetMgr.getSelfData().liangDao){
					...
					this.scheduleOnce(this.autoHu,2);
				}
	...
	initMopai: function (seatIndex, pai) {
		...
		if(pai==null)
		{
			...
			nc.children[1].getComponent(cc.sprite).spriteFrame =
	...
	checkLiang: function () {
		...
		if(seatData.liangDao||seatData.holds==null){
			return;
		}
